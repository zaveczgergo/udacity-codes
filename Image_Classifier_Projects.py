...
def load_checkpoint(filepath):
    ...
    model_string = "models." + architecture + "(pretrained=True)"
...
def process_image(image):
    ...
    im = Image.open(image)
    im_transforms = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    im_final = im_transforms(im)
    return im_final
...
image = Image.open("flowers/test/102/image_08004.jpg")
...
probabilities, cat = predict("flowers/test/102/image_08004.jpg", "checkpoint.pth")
...
image = "flowers/test/102/image_08004.jpg"

fig, axs = plt.subplots(2)

imshow(process_image(image), ax=axs[0])

probabilities, cat = predict(image, "checkpoint.pth")

names = [cat_to_name[i] for i in cat]
axs[1].barh(names, probabilities)
plt.show()
